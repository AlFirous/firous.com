// Source: https://cloud.umami.is/script.js (need update if there's change from the source)
...
!(function () {
  ...
  ((t) => {
    ...
    const y = "data-",
      ...
      M = w("auto-track") !== v,
      N = w("do-not-track") === b,
      O = w("exclude-search") === b,
      $ = w("exclude-hash") === b,
      j = w("domains") || "",
      A = w("fetch-credentials") || "omit",
      L = w("performance") === b,
      I = w("auto-pageview") !== v,
      P = j.split(",").map((t) => t.trim()),
      K = `${(x || "https://api-gateway.umami.dev").replace(/\/$/, "")}/api/send`,
      R = `${e}x${a}`,
      U = /data-umami-event-([\w-_]+)/,
      _ = `${y}umami-event`,
      B = 300,
      D = (t) => {
        if (!t) return t;
        try {
          ...
          return (O && (e.search = ""), $ && (e.hash = ""), e.toString());
        } catch {
          return t;
        }
      },
      W = () => ({ website: S, screen: R, language: n, title: c.title, hostname: h, url: Y, referrer: Z, tag: k, id: V || void 0 }),
      C = (t, e, a) => {
        a && ("function" == typeof X && X(), (Z = Y), (Y = D(new URL(a, o.href).toString())), Y !== Z && I && setTimeout(F, B));
      },
      J = () =>
        et ||
        !S ||
      ...
      q = async (e, a = "event") => {
        ...
        if (("function" == typeof n && (e = await Promise.resolve(n(a, e))), e))
          try {
            const t = await fetch(K, { keepalive: !0, method: "POST", body: JSON.stringify({ type: a, payload: e }), headers: { "Content-Type": "application/json", ...(void 0 !== Q && { "x-umami-cache": Q }) }, credentials: A }),
              n = await t.json();
      ...
      z = () => {
        tt ||
          ...
          (() => {
            ...
            ((s.pushState = t(s, "pushState", C)), (s.replaceState = t(s, "replaceState", C)));
          })(),
          (() => {
            const t = async (t) => {
      ...
      F = (t, e) => q("string" == typeof t ? { ...W(), name: t, data: e } : "object" == typeof t ? { ...t } : "function" == typeof t ? t(W()) : W()),
      G = (t, e) => ("string" == typeof t && (V = t), (Q = ""), q({ ...W(), data: "object" == typeof t ? t : e }, "identify")),
      H = () => {
        const e = {};
        let a,
          n = !1,
          ...
          new PerformanceObserver((t) => {
            t.getEntries().forEach((t) => {
              if (t.interactionId) {
                ...
                if (n.length) {
                  const t = Math.floor(0.02 * Math.max(n.length, 10));
            ...
            n ||
              ((() => {
                if (r) {
                  ...
                  if (void 0 === e.lcp) {
                    const t = l("largest-contentful-paint"),
                      a = t?.[t.length - 1];
    ...
    t.umami || (t.umami = { track: F, identify: G, getSession: () => ({ cache: Q, website: S }) });
    let Q,
      V,
      X,
      Y = D(m),
      Z = D(l.startsWith(p) ? "" : l),
      tt = !1,
      et = !1;
    M && !J() && ("complete" === c.readyState ? z() : c.addEventListener("readystatechange", z, !0));
  })(window);
})();
